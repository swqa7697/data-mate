SHELL := /bin/bash
.DEFAULT_GOAL := help
.PHONY: help install build dev clean uninstall format tidy format-check lint test test-race test-integration
...
clean: ## Remove only reserved .build output; preserve installation and evidence
	@./scripts/clean.sh
