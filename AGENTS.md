...
- Use a dedicated operator-managed read-only PostgreSQL account and a read-only transaction for every PostgreSQL operation. Internal SQLite state changes use write transactions under the service's mutation and ownership contracts. Keep the PostgreSQL query guard limited to statement kind, direct relation scope, and explicit calls to core PostgreSQL routines. Reject direct application/extension routine calls; continue trusting indirect execution through database-defined views, operators, types, indexes, partitions and RLS. Catalog definitions are inspected without evaluating their expressions. Preserve resource bounds and do not reintroduce semantic manifests or exhaustive privilege audits.
